...
import org.jarcem.recognizer.decoder.pruner.Pruner;
import org.jarcem.recognizer.decoder.scorer.AcousticScorer;
import org.jarcem.recognizer.frontend.Data;
import org.jarcem.recognizer.linguist.Linguist;
...
import org.jarcem.recognizer.linguist.acoustic.tiedstate.Sphinx3Loader;
import org.jarcem.recognizer.linguist.allphone.PhoneHmmSearchState;
...
public class WordPruningBreadthFirstLookaheadSearchManager extends WordPruningBreadthFirstSearchManager {
     ...
     *
     * @param token the token to collect successors from be immediately expanded
    ...
    protected void collectSuccessorTokens(Token token) {
        ...
        boolean stateProducesPhoneHmms = state instanceof LexTreeLinguist.LexTreeNonEmittingHMMState || state instanceof LexTreeLinguist.LexTreeWordState
                || state instanceof LexTreeLinguist.LexTreeEndUnitState;
        for (SearchStateArc arc : arcs) {
            SearchState nextState = arc.getState();

            // prune states using lookahead heuristics
            if (stateProducesPhoneHmms) {
                if (nextState instanceof LexTreeLinguist.LexTreeHMMState) {
                    Float penalty;
